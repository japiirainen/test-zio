...
import com.zio.server.configuration.Configuration
import com.zio.server.http.Api
import com.zio.server.persistence.{
  DBTransactor,
  UserPersistence,
...
object Main extends App {
  ...
  override def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] = {
    val program: ZIO[AppEnvironment, Throwable, Unit] =
      for {
        _ <- UserPersistenceService.createUsersTable
